...
public class TestResultsAnalyzerActionTest {
	...
	@Test public void testGetTreeResult1() throws Exception {
		...
		assertEquals("{\"builds\":[\"2\",\"1\"],\"results\":[]}", jsonObj.toString());
	}
  
	@Test public void testGetTreeResult2() throws Exception {
		FreeStyleProject project = j.createFreeStyleProject();
		FreeStyleBuild build = project.scheduleBuild2(0).get();
		FreeStyleBuild build2 = project.scheduleBuild2(0).get();
		...
		assertEquals("{\"builds\":[\"4\",\"3\"],\"results\":[]}", jsonObj.toString());
		jsonObj = action.getTreeResult("5", "all");
		assertEquals("{\"builds\":[\"4\",\"3\",\"2\",\"1\"],\"results\":[]}", jsonObj.toString());
	}	
}
